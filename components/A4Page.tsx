...
import { ChevronLeft, ChevronRight, Mail, Menu, PanelRightClose } from 'lucide-react'; // Import Mail icon
    ...
    <div id={`page-${pageNumber}`} className="flex justify-center w-full py-8 px-2 sm:px-4">
      ...
      >
        ...
        <button
          ...
          className="absolute top-4 right-4 z-50 p-2 bg-white/80 backdrop-blur-sm hover:bg-slate-100 rounded-full text-slate-600 transition-all shadow-sm border border-slate-200"
          title={isSidebarOpen ? "Cerrar barra lateral" : "Abrir menú"}
        >
          {isSidebarOpen ? <PanelRightClose size={20} /> : <Menu size={20} />}
        </button>

        {/* Main Content Area (Left) */}
        <div className={`flex-1 flex flex-col p-8 pr-6 ${isSidebarOpen ? 'border-r border-slate-100' : ''}`}>
          {title && (
        ...
        {isSidebarOpen && (
          <div className="w-40 flex-shrink-0 flex flex-col py-6 px-2 bg-white">
